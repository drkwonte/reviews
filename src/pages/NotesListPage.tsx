      ...
      <div className="container py-10 px-4 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2 bg-card p-1.5 rounded-full border border-border shadow-sm overflow-x-auto">
              {subjects.map(sub => (
                <button 
                  key={sub} 
                  onClick={() => { setSelectedSubject(sub); setSearchParams({ subject: sub === '전체' ? '' : sub }) }} 
                  className={cn("px-6 py-2 rounded-full text-[11px] font-black transition-all whitespace-nowrap", selectedSubject === sub ? "bg-foreground text-background shadow-xl" : "bg-transparent text-muted-foreground hover:text-foreground hover:bg-muted")}
                >
                  {sub}
                </button>
              ))}
            </div>
            <Button variant="ghost" className={cn("h-11 px-5 rounded-full font-black text-[10px] uppercase tracking-widest transition-all", showFavorites ? "bg-primary/10 text-primary shadow-sm" : "text-slate-400 hover:text-slate-600")} onClick={() => setShowFavorites(!showFavorites)}>
              <Star size={12} className={cn("mr-2 transition-transform group-hover:scale-125", showFavorites ? "fill-current" : "")} />중요 문제만
            </Button>
          </div>
          <div className="relative w-full md:w-80 group">
            ...
            <Input placeholder="어떤 문제를 찾으시나요?" className="pl-12 h-12 bg-card border-border rounded-full focus:border-primary font-bold text-sm shadow-sm transition-all focus:ring-4 focus:ring-primary/10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredNotes.map((note) => {
              ...
              return (
                <div key={note.id} className="group bg-card rounded-[40px] border border-border overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer flex flex-col relative" onClick={() => navigate(`/notes/${note.id}`)}>
                  <div className="h-52 bg-muted relative overflow-hidden">
                    ...
                    <Badge className="absolute top-5 left-5 bg-slate-900/80 backdrop-blur-md text-white border-0 font-black px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest shadow-lg">{note.subject}</Badge>
                    {note.is_favorite && <div className="absolute top-5 right-5 text-primary drop-shadow-md"><Star size={20} fill="currentColor" /></div>}
                  </div>
                  <div className="p-8">
                    <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">{note.source || '직접 등록'}</p>
                    <h3 className="font-extrabold text-foreground text-lg line-clamp-1 mb-5">{note.category || '단원 미상'}</h3>
                    <div className="flex items-center justify-between pt-5 border-t border-border">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-0.5">복습 정답률</span>
                        <span className={cn("font-black leading-none", hasReviewed ? "text-xl text-primary" : "text-sm text-muted-foreground italic")}>
                          {hasReviewed ? `${displayAccuracy}%` : '복습 시작하지 않음'}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                         <button onClick={(e) => toggleFavorite(e, note)} className={cn("p-2 rounded-full transition-all hover:scale-110", note.is_favorite ? "text-primary hover:bg-primary/10" : "text-muted-foreground hover:bg-muted")}><Star size={20} fill={note.is_favorite ? "currentColor" : "none"} /></button>
                         <button onClick={(e) => handleDelete(e, note.id)} className="p-2 rounded-full text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all hover:scale-110"><Trash2 size={20} /></button>
                         <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center ml-2 border border-border group-hover:bg-foreground group-hover:border-foreground transition-all">
                           <ChevronRight size={18} className="text-muted-foreground group-hover:text-background transition-all transform group-hover:translate-x-0.5" />
                         </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="text-center py-40 bg-card rounded-[40px] border-2 border-dashed border-border flex flex-col items-center">
            ...
            <p className="text-muted-foreground font-bold mt-2">조건에 맞는 문제가 없거나 아직 등록된 문제가 없습니다.</p>
          </div>
        )}
      </div>
    </div>
